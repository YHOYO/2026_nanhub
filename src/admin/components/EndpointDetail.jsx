...
/**
 * EndpointDetail - Detailed view for a specific endpoint
 * Shows: summary KPIs, daily trend, usage by model, by project, by API key, cache/savings metrics
 */
...
const EndpointDetail = ({ endpoint, period, onBack }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/dashboard/super/endpoint/${encodeURIComponent(endpoint)}?period=${period}`);
  ...
  if (loading) {
    return (
      <div style={styles.container}>
        ...
        <div style={styles.loading}>Cargando detalle del endpoint...</div>
      </div>
    );
  }
  ...
  const maxProjectTokens = Math.max(...byProject.map(p => p.tokens || 0), 1);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}>
            ...
            <span style={styles.endpointBadge}>{endpoint}</span>
          </h1>
      ...
      {/* Charts: Daily Trend for this endpoint */}
            ...
            <tbody>
              {byProject.map((p, i) => (
                <tr key={p.projectId || i} style={{ ...styles.trHover, cursor: 'pointer' }} onClick={() => window.dispatchEvent(new CustomEvent('navigate-to-project', { detail: { projectId: p.projectId, projectName: p.projectName } }))}>
                  ...
                  <td style={styles.tdRight}>{formatNumber(p.requests)}</td>
                  <td style={styles.tdRight}>{formatNumber(p.tokens)}</td>
                  <td style={styles.tdRight}>
                  ...
                  <td style={styles.td}>
                    <div style={styles.bar}>
                      <div style={{
                        ...styles.barFill,
                        width: `${((p.tokens || 0) / maxProjectTokens * 100)}%`,
                        background: BAR_COLORS[i % BAR_COLORS.length],
                      }} />
                    </div>
                  </td>
      ...
      {/* Empty state */}
      {byModel.length === 0 && byProject.length === 0 && byApiKey.length === 0 && dailyTrend.length === 0 && (
        <div style={styles.section}>
          <div style={styles.empty}>
            No hay datos de uso para el endpoint <strong>{endpoint}</strong> en este período.
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: '16px', marginTop: '16px', fontSize: '12px', color: '#64748b' }}>
        ...
        <span>Haz clic en un <span style={{ color: '#22d3ee' }}>modelo</span>, <span style={{ color: '#22d3ee' }}>proyecto</span> o <span style={{ color: '#22d3ee' }}>API KEY</span> para ver el detalle</span>
      </div>
    </div>
  );
};
